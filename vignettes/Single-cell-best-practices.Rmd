...
# Project setup, organization, and reproducibility
...
https://github.com/yerkes-gencore/rstudio-server. Using the gencore dockerized Rstudio
...
```
podman run -e PASSWORD=password -e ROOT=TRUE -p 8787:8787 gencore/rstudio:4.2.1
```
...
## GencoreSC versioning

With `devtools::install_github()`, you can install any branch or version of the package. This can be 
especially useful in the early stages as we continue to develop the package in real-time, but eventually
it would be a good idea to refer to a stable version in your script (and hopefully your `renv` lockfile too).

Adding `build_vignettes = TRUE` to the `install_github()` call will build the 
vignettes as well. 
...
See the help documentation for `runIntegration()` for more details.

The result of integration can be conveniently visualized via the function
`plotIntegrationDiagnostics()`. See the help documentation for more details. 
...
`HTODemux()`. See the function documentation for more details. 
